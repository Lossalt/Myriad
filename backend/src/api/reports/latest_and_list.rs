...
/// A platform already being generated by any entry point is skipped there.
fn spawn_report_auto_regen(db: DatabaseConnection, user_id: i32, platforms: Vec<String>) {
    if platforms.is_empty() {
        return;
    }
    tokio::spawn(async move {
        tracing::info!("♻️ Auto-regenerating expired reports: {:?}", platforms);
        let (_reports, skipped) =
            generate_platform_reports_internal(&db, user_id, platforms, None).await;
        if !skipped.is_empty() {
