//! 联邦体积上限的单一事实源。数字必须同一处，否则本端发得出、对端 inbox 拒收。
//!
//! # 这条链
//!
//! ```text
...
//! （≤ [`inbox_body_limit`]，默认 [`INBOX_BODY_LIMIT`]）+ 解析后的 `serde_json::Value`（结构化开销通常是
...
//! 2. [`inbox_inflight_raw_budget`]（默认 [`INBOX_INFLIGHT_RAW_BUDGET`]）限制**并发**缓冲的原始 body 总量；预算耗尽
//!    返回 429。
...
///
/// 默认 4 MiB；运行时 [`message_payload_limit`]（memory-saver 可降）。
/// 公共 inbox 只承载活动与小型内联数据；大文件走分块传输。
...
///
/// 必须容纳 [`MESSAGE_PAYLOAD_LIMIT`] 加上活动信封与 JSON/base64 膨胀。
/// 分块传输不走 inbox（见 [`TRANSFER_CHUNK_BODY_LIMIT`]）。
///
/// 默认 8 MiB（相对默认 4 MiB payload + 信封/JSON/base64）。
/// 运行时 [`inbox_body_limit`]（memory-saver 可降）。更大内容走分块传输。
...
/// 已认证用户提交内容的路由上限（发布、房间/频道消息、关注等）。
/// Note 媒体走 [`live_note_media_body_limit`]，不走本上限。
///
/// 比 inbox 略宽：这些请求来自已登录的本地用户，不是任意远端。
/// 默认比 inbox const 多 16 MiB。运行时 [`authenticated_body_limit`]（saver 另算）。
/// 公开 inbox 不继承该宽限。
...
///
/// 这些端点的 body 只有几百字节到几 KiB。256 KiB 足够容纳异常大的
...
/// 6 MiB，无 live 包装。按默认 4 MiB payload 的 base64/加密膨胀留余量。
...
/// amplification is bounded separately (`MAX_CONCURRENT_TRANSFERS`) rather than shrinking this.
pub const MAX_FILE_SIZE: i64 = 20 * 1024 * 1024 * 1024;

// ── transfer amplification admission (generous budgets) ───────────
//
...
/// Max concurrent open transfers (`pending` + `in-progress` + `finalizing`) process-wide.
...
///
/// Sum of open-transfer `file_size` must stay under this (`MAX_FILE_SIZE` is 20 GiB).
...
/// 单条 Note 的正文字符数上限（100_000 个 Unicode scalar）。
...
//
// Default-profile numbers (memory-saver lowers live inbox/inflight caps):
//
...
impl InboxInflightPermit {
    /// Bytes this permit holds against the live [`inbox_inflight_raw_budget`].
    #[cfg(test)]
...
///
/// `bytes` is clamped to `1..=inbox_body_limit()` (live cap; default equals
/// [`INBOX_BODY_LIMIT`]). Zero-length still reserves 1.
///
...
/// Reserve concurrent budget from `Content-Length` (or the live inbox limit if
/// absent / unparseable), then buffer up to [`inbox_body_limit`] (default [`INBOX_BODY_LIMIT`]).
///
...
mod tests {
    ...
    /// 实证 axum 的 layer 覆盖语义：离 handler 更近的 `DefaultBodyLimit` 覆盖外层。
    ///
    /// 本测试只钉这一层语义。生产联邦路由用 `live_*_body_limit`（`from_fn`），
    /// 嵌套路由不再 `set DefaultBodyLimit`。inbox / authenticated 比全局 50 MiB 更窄。
    #[tokio::test]
    async fn inner_body_limit_overrides_the_outer_one() {
        ...
        // 内层 DefaultBodyLimit 比外层更宽（仅本测试；生产联邦不靠这条语义放行超 50 MiB）
...
mod constant_tests {
    ...
    /// Concurrent raw budget
    /// (`INBOX_INFLIGHT_RAW_BUDGET` 32 MiB × 3 ≈ 96 MiB parse peak) must stay
    /// under 1 GiB / 4. Changing these is a product/profile decision — this
...
mod optional_json_tests {
    ...
    ///
    /// `federation_join_room` 的 body 可选。`Option<Json<T>>` 下空 body 与畸形 JSON 必须实测。
    #[tokio::test]
...
#[cfg(test)]
mod rejection_tests {
    /// 超限 body 必须仍然拿到带分块传输指引的 413，不能只剩 axum 纯文本 413。
    #[tokio::test]
