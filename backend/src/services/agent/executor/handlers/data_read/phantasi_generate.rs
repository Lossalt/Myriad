...
/// stays `phantasi:read` only. Autonomy caps, when present, must also include it.
...
pub(super) async fn execute_phantasi_generate_reading_list(
    ...
    tracing::info!(
        ...
        "[phantasi.generateReadingList] Parameters parsed"
    );
    ...
    let mut items: Vec<ReadingListCandidate> = if !keyword.is_empty() {
        ...
        tracing::info!(
            ...
            "[phantasi.generateReadingList] Keyword search results"
        );
        ...
        if keyword_items.is_empty() {
            tracing::info!(
                keyword = %keyword,
                allow_web_search = allow_web_search,
                "[phantasi.generateReadingList] No keyword matches in local phantasi_items"
            );
            vec![]
            ...
            tracing::info!(
                keyword = %keyword,
                found = keyword_items.len(),
                "[phantasi.generateReadingList] Keyword results insufficient, fetching more articles"
            );
    ...
    if needs_more {
        ...
        if allow_web_search && (!keyword.is_empty() || !criteria.is_empty()) {
            tracing::info!(
                ...
                "[phantasi.generateReadingList] allowWebSearch=true, trying AI web search"
            );
            ...
            .await
            {
                Ok(web_results) if !web_results.is_empty() => {
                    tracing::info!(
                        ...
                        "[phantasi.generateReadingList] AI web search returned results"
                    );
                    return Ok(json!({
        ...
        if !items.is_empty() {
            tracing::info!(
                keyword = %keyword,
                db_results = items.len(),
                "[phantasi.generateReadingList] Thin local results, ranking without web search"
            );
        } else {
            tracing::info!(
                keyword = %keyword,
                allow_web_search = false,
                "[phantasi.generateReadingList] Local keyword empty; honest empty (no forced webSearch)"
            );
            ...
            let mut suggestions = vec![
                ...
                "Browse local feeds with phantasi.items / phantasi.read".to_string(),
...
JSON only."#,
    ...
    tracing::info!(
        ...
        "[phantasi.generateReadingList] Generated reading list"
    );
