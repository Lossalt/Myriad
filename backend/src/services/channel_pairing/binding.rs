...
mod postgres_tests {
    ...
    async fn pairing_revocation_and_rotation_use_real_postgres() {
        ...
        db.execute_unprepared("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, username TEXT, is_admin BOOLEAN, is_owner BOOLEAN DEFAULT false); \
            ...
            CREATE TABLE IF NOT EXISTS runtime_registry (namespace TEXT, record_id TEXT, subject_id INTEGER, owner_id INTEGER, tapp_id TEXT, runtime_id TEXT, payload JSONB NOT NULL, expires_at BIGINT, updated_at TIMESTAMPTZ DEFAULT NOW(), PRIMARY KEY(namespace, record_id)); \
            CREATE TABLE IF NOT EXISTS runtime_mailbox (expires_at BIGINT); \
            TRUNCATE user_identities, runtime_registry, users RESTART IDENTITY; \
