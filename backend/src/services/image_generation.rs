...
//!
//! Caller passes width/height (clamped 256–2048); `image_size_param` maps them per model.
//! Generated bytes are persisted as platform media assets so Agent / Tapp
//! surfaces get a stable URL instead of a short-lived provider link.
...
pub struct ImageReference {
    /// Shared handle: bundled references stay on their static backing bytes and
    /// multipart uploads hand the same buffer to the request body.
    pub bytes: Bytes,
    pub media_type: String,
}
...
pub fn config_from_dynamic(
    ...
    let (api_key, base_url) = if let Some(source) = source.as_ref() {
        ...
        match source.kind.trim().to_ascii_lowercase().as_str() {
            ...
            "gemini" => (api_key, gemini_image_base_url(dynamic, Some(source))),
...
fn image_edit_form(
    ...
    for reference in references {
        let part = reqwest::multipart::Part::stream_with_length(
            reqwest::Body::from(reference.bytes.clone()),
            reference.bytes.len() as u64,
        )
...
/// Load a local reference without an outbound request.
...
pub async fn remove_persisted_generated(
    ...
    if let Some(id) = persisted.asset_id {
        match MediaService::from_data_paths(paths()).delete(db, id).await {
            Ok(_) => Ok(()),
            Err(error) => Err(ImageGenerationError::Provider(error.to_string())),
        }
    } else {
...
mod tests {
    ...
    #[test]
    fn persist_generated_writes_assets_not_cache() {
        let src = include_str!("image_generation.rs");
        let persist = src
            .split("pub async fn persist_generated_with_status")
            .nth(1)
            .expect("persist fn");
        let persist = persist
            .split("pub async fn remove_persisted_generated")
            .next()
            .unwrap();
        assert!(persist.contains("MediaService"));
        assert!(persist.contains("persist_ready_bytes"));
        assert!(!persist.contains("store_bytes_with_status"));
    }
}
