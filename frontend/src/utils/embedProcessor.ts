...
function generateNeteaseMusicCard(songId: string): string {
  return `
    <div class="phantasi-embed-card phantasi-netease-music phantasi-embed-exempt not-prose block group cursor-pointer"
      ...
      <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-md hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 hover:scale-[1.02] overflow-hidden h-full">
        <div class="block w-full h-full relative">
          <!-- 封面容器 - 可被动态更新 -->
          <div class="phantasi-embed-cover w-full h-full bg-linear-to-br from-red-400 to-red-600 flex items-center justify-center">
            <svg class="w-16 h-16 text-white/80 phantasi-embed-placeholder" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div class="absolute inset-0 bg-linear-to-t from-black/95 via-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-3 pointer-events-none">
            <div>
              <div class="flex items-start gap-1">
                <h3 class="phantasi-embed-title font-bold text-white text-xs leading-tight line-clamp-2 mb-1 flex-1">
                  ${currentCopy().common.loading}
                </h3>
              </div>
              <p class="phantasi-embed-artist text-[10px] text-white/75 line-clamp-1">
                ID: ${songId}
              </p>
            </div>
          </div>
...
function generateSteamGameCard(appId: string): string {
  ...
  return `
    <div class="phantasi-embed-card phantasi-steam-game phantasi-embed-exempt not-prose block group"
      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden">
        <div class="relative overflow-hidden bg-linear-to-br from-gray-900 to-gray-800">
          <a href="${storeUrl}"
            ...
            <img src="${headerImg}"
                 alt="Steam Game"
                 class="phantasi-embed-cover w-full block object-cover aspect-460/215 transition-all duration-500 group-hover:scale-110"
                 loading="lazy"/>
          </a>
          ...
          <div class="absolute inset-0 bg-linear-to-t from-black/90 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4 pointer-events-none">
            <h3 class="phantasi-embed-title font-bold text-white text-base line-clamp-2 leading-snug mb-1">
              App ID: ${appId}
            </h3>
            <p class="phantasi-embed-desc text-sm text-white/80 line-clamp-1">
              ${currentCopy().phantasi.embedSteamStore}
            </p>
          </div>
        </div>
      </div>
...
function generateBilibiliIframe(videoId: {
  ...
  return `
    <div class="phantasi-embed-card phantasi-bilibili-embed phantasi-embed-exempt not-prose block"
...
function generateGithubRepoCard(repo: { owner: string; repo: string }): string {
  ...
  return `
    <div class="phantasi-embed-card phantasi-github-repo phantasi-embed-exempt not-prose block"
      ...
      <a href="${repoUrl}"
         target="_blank"
         rel="noopener noreferrer"
         class="phantasi-github-card">
        <div class="phantasi-github-card-header">
          <span class="phantasi-github-card-avatar" aria-hidden="true">
            <img src="${ownerAvatar}" alt="" loading="lazy" />
          </span>
          <span class="phantasi-github-card-text">
            <span class="phantasi-github-card-title phantasi-embed-title">${repo.repo}</span>
            <span class="phantasi-github-card-owner phantasi-embed-owner">${repo.owner}</span>
          </span>
          <span class="phantasi-github-card-stars phantasi-embed-stars">
        ...
        <p class="phantasi-github-card-desc phantasi-embed-desc"></p>
        <div class="phantasi-github-card-meta">
          <span class="phantasi-embed-lang">
            <span class="phantasi-embed-lang-dot"></span>
            <span>-</span>
          </span>
          <span class="phantasi-embed-forks">
            <svg fill="currentColor" viewBox="0 0 16 16" aria-hidden="true">
        ...
        <span class="phantasi-github-card-mark" aria-hidden="true">
...
function generateGithubRepoChip(repo: { owner: string; repo: string }): string {
  const repoUrl = `https://github.com/${repo.owner}/${repo.repo}`
  const ownerAvatar = `https://github.com/${repo.owner}.png?size=48`
  return `<a class="github-project-badge phantasi-github-chip phantasi-embed-exempt"
       href="${repoUrl}"
       target="_blank"
       rel="noopener noreferrer"
       data-embed-type="github-repo"
    ...
    <span class="github-project-badge-mark phantasi-github-chip-avatar" aria-hidden="true">
      <img src="${ownerAvatar}" alt="" loading="lazy" />
    </span>
...
async function loadNeteaseMusicData(container: HTMLElement): Promise<void> {
  const unloadedCards = Iterator.from(
    container.querySelectorAll('.phantasi-netease-music[data-song-id]'),
  )
    .filter(
      (card) =>
  ...
  await Promise.all(
    unloadedCards.map(async (card) => {
      ...
      try {
        ...
        const coverContainer = card.querySelector('.phantasi-embed-cover')
        ...
        const titleEl = card.querySelector('.phantasi-embed-title')
        if (titleEl) {
          titleEl.textContent = songData.name
        }

        const artistEl = card.querySelector('.phantasi-embed-artist')
        if (artistEl) {
        ...
        if (songData.isVip || songData.fee === 1 || songData.fee === 4) {
          const titleContainer =
            card.querySelector('.phantasi-embed-title')?.parentElement
...
async function loadSteamGameData(container: HTMLElement): Promise<void> {
  const unloadedCards = Iterator.from(
    container.querySelectorAll('.phantasi-steam-game[data-app-id]'),
  )
    .filter(
      (card) =>
    ...
    unloadedCards.map(async (card) => {
      ...
      try {
        ...
        if (gameData) {
          ...
          const descEl = card.querySelector('.phantasi-embed-desc')
...
async function loadGithubRepoData(container: HTMLElement): Promise<void> {
  const unloadedCards = Iterator.from(
    container.querySelectorAll('.phantasi-github-repo[data-owner][data-repo]'),
  )
    .filter((card) => {
  ...
  await Promise.all(
    unloadedCards.map(async (card) => {
      ...
      try {
        ...
        const descEl = card.querySelector('.phantasi-embed-desc')
        if (descEl) {
          descEl.textContent = currentCopy().phantasi.repoLoadFailed
        }
